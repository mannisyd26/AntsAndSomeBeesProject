...
def extract_words(text):
    ...
    return final_text.split()  # Replace this line
...
def analyze_tweet_sentiment(tweet):
    ...
    for i in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(i)):
            total = total + sentiment_value(get_word_sentiment(i))
            sentiments += 1
    if sentiments == 0:
        average = make_sentiment(None)
    else:
        average = make_sentiment(total/sentiments)
...
def find_centroid(polygon):
    ...
    for j in range(0, len(polygon)-1):
        x_centroid = x_centroid + (latitude(polygon[j]) + latitude(polygon[j+1])) * (latitude(polygon[j]) * longitude(polygon[j+1]) - latitude(polygon[j+1]) * longitude(polygon[j]))
        y_centroid = y_centroid + (longitude(polygon[j]) + longitude(polygon[j+1])) * (latitude(polygon[j]) * longitude(polygon[j+1]) - latitude(polygon[j+1]) * longitude(polygon[j]))
    if area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), 0)
    x_centroid = x_centroid / (6 * area)
    y_centroid = y_centroid / (6 * area)
...
def find_state_center(polygons):
    ...
    for i in range(0,len(polygons)):
        x += find_centroid(polygons[i])[0] * find_centroid(polygons[i])[2]
        y += find_centroid(polygons[i])[1] * find_centroid(polygons[i])[2]
        area += find_centroid(polygons[i])[2]
...
def group_tweets_by_state(tweets):
    ...
    for i in tweets:
      ...
      for state in us_states.keys():
        distance = geo_distance(find_state_center(us_states[state]),tweet_location(i))
        if close == 0 or distance < close:
          close = distance
          close_state = state
      if close_state in tweets_by_state:
        tweets_by_state[close_state].append(i)
      else:
        tweets_by_state[close_state] = [i]
...
def average_sentiments(tweets_by_state):
    ...
    for i in tweets_by_state.keys():
        tweets = tweets_by_state[i]
        total = 0
        for j in range(len(tweets)):
            if has_sentiment(analyze_tweet_sentiment(tweets[j])) != False:
                tweets[j] = sentiment_value(analyze_tweet_sentiment(tweets[j]))
                total += 1
            else:
                tweets[j] = 0
        if total != 0:
            count = sum(tweets)
            averaged_state_sentiments[i] = count/total
